...
9. **远程优先核验与完成通知**：核验任何"已完成/已签署/已合入"声称时，先 `git fetch origin` 再按声称类型查远端对应层级（存在性 R1 = 远端分支/PR 可见；生效性 R2 = 已合入 `main`），禁止仅凭本地 worktree 判定；完成通知必须携带远端状态证据（实施提交 SHA、回执提交 SHA、PR 号、R2 状态）。执行回执采用双轨（Markdown 正文 + 同目录 `*-summary.json` 结构化摘要，模板见 `handoff/backend-to-frontend/_template-task-receipt-summary.json`）。细节与分级补救见 `docs/agents/verification-workflow.md`。
