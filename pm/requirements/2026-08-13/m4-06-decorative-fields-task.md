...
- 后端评审 负责撰写提案并写入 `proposals/backend/<日期>/m4-06-decorative-fields-proposal.md`（后端实现提供运行时实现事实与影响评估输入；提案写入遵循 `AGENTS.md` 角色边界；选定方案记录在提案文内或 PM 决策中）。
- 提案必须评估两案并给出推荐方案：
...
- 关闭条件（全部满足后 PM 复核关闭 M4-06）：
  ...
  4. 回执含提案编号、PM 确认记录、提交哈希与测试输出，后端评审 复核签署。
